...
**CSP живёт в двух местах** и должен быть синхронным: `nginx/nginx.conf` (главный — действует на страницу) и helmet в [app.ts](mrdk-back/src/app.ts) (только ответы API). Внешние домены — Яндекс.Метрика (script/connect с `wss://`/img/frame) и карта: она встроена как iframe Яндекс.Конструктора, поэтому от неё нужен только `frame-src https://yandex.ru` — тайлы грузит документ внутри фрейма, на него наша политика не действует. `'unsafe-eval'` в `script-src` больше нет: он стоял там ради Raster JS API 2ГИС и ушёл вместе с ним.

...
# 1 = снять бэкап перед up (миграции накатятся на старте). Присваивание через
# :- , чтобы работал разовый вызов `BACKUP_BEFORE=1 ./deploy.sh` перед рискованной
# миграцией: безусловное BACKUP_BEFORE=0 затирало бы переменную окружения, и бэкап
# молча не снимался бы именно там, где он нужен.
